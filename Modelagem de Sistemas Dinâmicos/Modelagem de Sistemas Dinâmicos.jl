...
# ╔═╡ d0ada492-ce96-4930-8dca-b2ea8dd8c732
begin
...
# ╔═╡ 69f6a98a-3540-11ed-39fe-39bbe12d5ac7
md"""
# Modelagem e representação de sistemas dinâmicos
"""

# ╔═╡ 710b64fd-5be3-431a-a799-e3412613feea
md"""
## 1. Do livro DORF, exercício E3.12, para os valores de R, L, C faça:
"""

# ╔═╡ 88dba8af-092b-4024-9ac4-0ac41dc4a033
LocalResource("E3.12.png")

# ╔═╡ 5197bf4a-858c-4717-a025-5d49aa10c0a9
begin
    r = 2
    l = 0.1
    c = 800e-6
...
# ╔═╡ f46a5f66-fd77-4950-846e-00d0017d616f
md"""
### a. Plote um gráfico da equação diferencial ordinária que modela o problema
```math
\begin{equation}
\ddot{V_c} = \frac{V_{en} - RC \dot{V_c} - V_c}{LC}
\end{equation}
```
   !!! não funcionou
"""

# ╔═╡ 1b582d0b-ba57-4fb2-acd1-22d6036d463a
md"""
primeiro vamos definir os valores de algumas constantes
"""

# ╔═╡ 4a3d83b8-06f1-4da5-9f2a-ba72222518ba
begin
    u₀ = 5e-3
    du₀ = 0.0
    time = (0.0, 0.6)
    ts = LinRange(time[1], time[2], 200)
end

# ╔═╡ cbaa0684-eff2-47a2-91ee-6c8b3fbf0a83
begin
    f1(du, u, p, t) = -(r / l) * du - 1 / (l * c) * u
    prob = SecondOrderODEProblem(f1, du₀, u₀, time)
    sol = solve(prob)

    z = sol(ts, Val{0})   # z  = (u' , u )
    dz = sol(ts, Val{1})  # z' = (u'', u')
    plot(ts,
        [z[2, :] z[1, :] dz[1, :]]
    )
end

# ╔═╡ ee800d38-6960-41bb-a521-ee8ca02af525
md"""
### b. Expresse funções de transferência da tensão no capacitor em relação à tensão de entrada
"""

# ╔═╡ 911b0e1d-353d-4b7b-b2b8-c8dca9348139
md"""
Função de Transferência da tensão no capacitor em relação à tensão de entrada
```math
\begin{equation}
\frac{V_c}{V_i} = \frac{1}{LCS^2 + RCS + 1}
\end{equation}
```
"""

# ╔═╡ 13e5192a-7340-4685-9c5a-89d8931c1dea
ft = tf([0, 0, 1], [l * c, r * c, 1])

# ╔═╡ eff31079-c56e-4fc4-90a4-89c13a382183
md"""
## 2. Considerando a representação em espaço de estados
"""

# ╔═╡ c5c2a7eb-8a53-427c-b83e-1eb77412bd7a
md"""
### a.	Obtenha as matrizes de estado
"""

# ╔═╡ 1e9f2579-2d3d-48a0-ad46-93a6c6221184
begin
    A = [
        0 (1/c)
        -(1 / l) -(r / l)
...
# ╔═╡ b6f50776-6f48-47a9-ab12-8b1bb3ae1d4a
md"""
### b.	Plote os gráficos da matriz de estados
"""

# ╔═╡ b5a07ff3-2f6b-4564-8956-2d526de3fbe6
md"""
definindo espaços de estados
"""

# ╔═╡ c10a5fa8-c7c5-4bb1-9c62-2839f20f32e6
sys = ss(A, B, C, D)

# ╔═╡ dcea6bdf-55ba-4bbd-a08b-00b212f4fa6d
md"""
plotando
"""

# ╔═╡ 492094b7-53fa-4a2c-94ad-3c30ad040591
begin
    y, t, x = step(sys, 0.6)
    plot(t, x';
        lab=["Tensão no  Capacitor [V]" "Corrente [A]"],
        xlabel="Tempo [s]"
    )
end

# ╔═╡ 267b65cd-c7c1-4df0-ba0e-94295755bf06
md"""
## 3. Altere aleatoriamente os valores de R, L e C e refaça 1 e 2.
"""

# ╔═╡ 4e3f7f79-aa10-457f-bd1f-7adfbba69b8a
begin
    r2 = 3
    l2 = 0.2
    c2 = 600e-6
...
# ╔═╡ 2d2af7f8-f204-430b-a7a6-158a66e4510f
md"""
### 1
"""

# ╔═╡ 9b9e0b13-4009-4895-8720-2711e0071e28
begin
    f2(du, u, p, t) = -(r2 / l2) * du - 1 / (l2 * c2) * u
    prob2 = SecondOrderODEProblem(f2, du₀, u₀, time)
    sol2 = solve(prob2)
    z2 = sol2(ts, Val{0})   # z  = (u' , u )
    dz2 = sol2(ts, Val{1})  # z' = (u'', u')
    plot(ts,
        [z2[2, :] z2[1, :] dz2[1, :]]
    )
end

# ╔═╡ 8467cbe4-f4b4-410b-827b-68719fc319bb
md"""
### 2
"""

# ╔═╡ 22823ad4-a99a-431c-b645-411b54c53ecb
begin
    A2 = [
        0 (1/c2)
        -(1 / l2) -(r2 / l2)
...
# ╔═╡ 409dd728-ed4e-4bc8-89b6-90d2e4e13c60
# Schilab
# # specs
# # Calcular os polos
# poles(sys)
...
julia_version = "1.8.5"
manifest_format = "2.0"
project_hash = "a375275c8a57ebb41b4cf3db0b7a2fb56f41a953"
